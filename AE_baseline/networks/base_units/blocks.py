"""
Building Blocks for AE/AE-U Networks
"""
...
class BasicBlock(nn.Module):
    """Basic convolutional block with optional downsampling/upsampling"""
...
class BottleNeck(nn.Module):
    """Bottleneck layer with linear encoding/decoding"""
...
class SpatialBottleNeck(nn.Module):
    """
    Spatial bottleneck using 1x1 convolutions
    
    TODO [향후 개선]: Positional encoding을 여기에 추가 가능
    """
...
class VaeBottleNeck(BottleNeck):
    ...
    Extends BottleNeck to support variational inference with reparameterization trick
    """
    
    def __init__(self, in_planes, feature_size, mid_num=2048, latent_size=16):
        super(VaeBottleNeck, self).__init__(in_planes, feature_size, mid_num, latent_size)
        # Override encoder to output 2*latent_size (mu and log_var)
        self.linear_enc = nn.Sequential(
            nn.Linear(in_planes * feature_size * feature_size, mid_num),
            nn.BatchNorm1d(mid_num),
            nn.ReLU(True),
